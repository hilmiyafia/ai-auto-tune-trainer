...
2. Label your audio samples. 
    * The labels should be separated by notes, **not** by phonemes like in DiffSinger. 
    * Do **not** label breaths.
3. Convert the labels to USTs. 
    * Make sure that the notes in the USTs are on the correct pitch (not flat).
    * Make sure that each UST has the same file name to its corresponding audio sample.
